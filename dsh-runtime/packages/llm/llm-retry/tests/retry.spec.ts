...
describe('provider-routed retry policy', () => {
  ...
  it('uses a bounded provider Retry-After verbatim and caps an over-cap instruction to maxDelayMs', async () => {
    vi.useFakeTimers()
    ...
    await context.fiber.dispose()
    const capped = new ScriptedAdapter([
      new LlmError('wait too long', 'RATE_LIMIT', { providerRetryAfterMs: 30_000 }),
      textResponse('done'),
    ])
    ;({ ctx: context } = await harness(capped))
    const cappedAgent = await context.agentLoop.create(SessionId('retry-after-capped'), { provider: 'mock', model: 'mock' })
    const cappedScheduled = waitForRetry(context, cappedAgent, 1)
    cappedAgent.followup(createUserMessage({ content: [{ type: 'text', text: 'go' }], source: { kind: 'user' } }))
    expect((await cappedScheduled).data.delayMs).toBe(10_000)
    const cappedIdle = waitForIdle(context, cappedAgent)
    await vi.advanceTimersByTimeAsync(10_000)
    await cappedIdle
    expect(capped.requests).toHaveLength(2)
  })

  it('caps an over-cap Retry-After to maxDelayMs in always mode without jitter', async () => {
    vi.useFakeTimers()
    const adapter = new ScriptedAdapter([
