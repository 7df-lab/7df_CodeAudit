  ...
  it('exposes a clean partial EOF as non-default-retryable STREAM_CLOSED', async () => {
    ...
    expect(agent.session.snapshotEvents().at(-1)).toMatchObject({
      ...
      data: {
        reason: {
          kind: 'error',
          // Assert the stable prefix: the adapter appends parseSse attribution
          // (event count, last frame age, provider host) after the base message.
          error: { message: expect.stringContaining('SSE stream ended without [DONE]'), code: 'STREAM_CLOSED' },
        },
      },
    })
  })
