...
**三层环境口径**(deploy/README.md;不允许宿主机裸栈开发测试,一律容器):
...
| 维度 | 内容 |
|------|------|
| **开发** | `npm run dev`(Vite 5173;`/v1` 代理到 `CODEAUDIT_GATEWAY_URL`,缺省 `http://localhost:8080`,WS 透传);`npm run dev:mock`(无后端视觉走查:挂 `src/testsupport/demoGateway.ts` 演示网关,设计整改 2026-09-13) |
| **测试** | `npm test`(Vitest + jsdom,2026-09-13 实测 31 文件 218 用例,随交付演进;`src/testsupport/fakeGateway.ts` 在 axios adapter 层造假网关);类型门禁在 `npm run build` 内(`tsc -b`);无 eslint。伞仓入口 `make test-web` / `make build-web` |
...
| **测试** | `npm test`(Mocha 11 + vscode 模块桩,263 用例(2026-09-13 实测),大头 applyPatch/extension 行为测试);真实链路脚本 `test/smoke.js`、`test/fixflow.e2e.js`、`test/mockGateway.js`(需网关在线) |
...
| 维度 | 内容 |
|------|------|
| **开发** | pnpm monorepo(53 包组,Node ≥22);`pnpm install` 后 `pnpm dsh` 可源码直跑;基线=上游 0.1.5-rc.2(`c291e7961a`,2026-09-13 GitHub 同步,合并提交 `194a2d17b0`);本地领先上游 **8 提交**=内部 7 个(`feb3854610` SSE 断流归因 + gitlab 线 6 个:接口契约三件套/回归账本门禁/plugin 转发器测试/.gitignore 禁 .agent//steering-history 清理/B7 审计加固)+合并提交自身;已知分歧:retry-after 封顶语义与 STREAM_CLOSED 归因后缀为本地有意保留 |
...
| **部署** | `./deploy.sh` = staging 组装 → `pct exec <CTID> docker build -t dsh-pentest-sse:latest` → 经 manager API 冒烟(`SKIP_SMOKE=1` 跳过;路由域断言 `EXPOSE_DOMAIN` env 参数化,缺省与 gateway `ROUTING_DOMAIN` 同源=`sandbox.codeaudit.internal`,2026-09-08);镜像无常驻进程,网关按需拉起 |
| **tag 口径** | 消费面一律 `:latest`();`deploy.sh` 缺省恒 `:latest`,版本化 `IMAGE=` 构建后自动刷新 latest;出新溯源 tag 时同步 `agent-tools-sbom.json` 的 image 字段(伞仓 toml 恒 `:latest`) |
...
- 模拟栈供给:`bash deploy/sim-sync.sh push|rebuild|test [VMID]`——把本地 git 已提交树(engine+web+deploy)同步到 107 侧检出台(/root/codeaudit-sim-check, git archive 只取已提交内容;收敛式同步——解包前清远端 engine/web 与 deploy 旧树,白名单保留 deploy/env.sim,上游已删文件不残留进构建上下文(2026-09-13 审计修))并调远端 sim.sh up/tests;rebuild 构建久, 勿套短超时;sim.sh up 自 2026-09-09 起自动保障沙箱南向链(宿主存在 openshell-manager/openshell-gateway 容器即拉起, 缺失显式警告降级口径; `sim.sh chain` 单独触发; dsh-runtime→manager:18800→gateway:8080 为 107 sim 独占依赖)(2026-09-06 固化;2026-09-07 起 web 与 engine 同批同步——console 容器 build context=../web,漏同步=前端修复不进部署产物,实证);up 末尾 seed 后自愈回拉 task/result——两服务 restart=no 且于 compose 拉起时即连业务库, 空卷首启(destroy 后首次 up/全新安装)必崩躺尸, 库就绪后回拉(已跑者 no-op 幂等; 不用 postgres initdb.d 挂载=ADR-111 否决在案)(2026-09-14 空卷完全重启实证)。
...
- 仓库拉取模式夹具:`bash deploy/tests/git_fixture_107.sh up|down|status`——107 上匿名 git-daemon(:19418, transient)+sample-sast.git(app.py: B608×2+B105),repo_url=`git://10.10.210.1:19418/sample-sast.git`(任务容器经 sim 网桥网关回宿主);验证 git clone 场景时 up→建 repo 项目 SAST_ONLY 任务→跑完 down 收敛(共享宿主不留常驻进程;2026-09-07 实证 findings=3)。
...
- web deploy 流程(2026-09-05 接管 CD 退役后的 console 发布职责):收敛式同步 web 源码树 → `web/docker-compose.yml` 单文件构建(`codeaudit-console:latest`,npm 构建在镜像内) → 8088→80,`CODEAUDIT_GATEWAY_UPSTREAM=host.docker.internal:8090` 反代宿主发布网关 → 健康等待 + `/v1` 401 透传反代检查(**五层验收门不在本脚本**——它在生产态一键 `production-deploy.sh` 的 `deploy_web`:console `/v1` 401 透传/SPA 深链回退+静态产物可达/nginx WS 升级头在位/经 console 源登录+带认证 200(admin 口令被改自动跳过保幂等)/上传体上限行为断言(2MB 放行防回落 1m 缺省,101MB 反代层 413——2026-09-13 审计勘误:31MB 为限值上调前旧值,engine `uploadMaxBytes=100<<20`)。
- **生产态一键**(`deploy/production-deploy.sh` + `deploy/production.env`,后者 gitignored,首跑按 template 自动生成:密钥随机+宿主 IP 探测):交互确认(`configure` 单独参数模式;`deploy` 开工前确认访问入口地址(**仅显示面,不参与接线**)/端口冲突/网段重叠,`--yes` 或非 TTY 跳过) → 预检 → opengrep 自动拉取(官方 release manylinux_x86 优先+三加速镜像依序兜底,sha256 统一复核,见 PROVENANCE.md) → 镜像预拉 → gateway(ensure 自足:JWT 密钥 generate-certs+supervisor 镜像 `:latest` retag `:local`) → manager(deploy/.manager-stage 装配) → engine → 沙箱镜像(DOCKER_CMD=docker) → web;`status/stop/down` 全套。密钥接线单文件:`deploy/production.env`(JWT/manager token/端口/网段/Kafka advertised 全部 env 化);**联动键每次运行自动重算回写**(manager→网关端点+沙箱拨号随 `OPENSHELL_PORT`、console 反代随 `CODEAUDIT_HOST_GATEWAY`,手改无效,2026-09-07 交互化时落地;`OPENSHELL_MANAGER_URL` 自 2026-09-08 起恒为内部常量 `http://host.docker.internal:18800`(带 scheme,引擎当 REST 基址)——manager 是内部面非交互面,与访问 IP 解耦,访问 IP 只存 `CODEAUDIT_ACCESS_IP` 供横幅显示);LLM provider 不在部署期配置(部署后按完成横幅指引自注册)。**Windows 支持**(2026-09-08):WSL2+Docker Desktop 路径,`deploy/windows/bootstrap.ps1` 环境引导后复用同一 bash 入口(镜像全 Linux 镜像,不维护第二套部署事实源;bash 脚本经临时文件/stdin 下发不经 PS 命令行——PS5.1 原生传参碎裂根治,静态门禁 `deploy/tests/bootstrap_static_check.sh`+crlf 合成仓 `deploy/tests/crlf_check_test.sh`;CRLF 哨兵/修复=deploy/windows/crlf_check.sh 两壳共有,退出码 0/1/2/3 契约);`expose-lan.ps1` 做 LAN 端口转发;bash 入口 WSL 下访问面缺省 `localhost`+提示 portproxy。
...
- e2e 套件 `bash deploy/tests/run.sh`(前置模拟栈已 up;`run.sh 04` 跑单用例;环境性跳过走 `check_skip` 如实计 `跳过=N` 不计失败——2026-09-13 前 stub 未就绪段误计 FAIL 挂红,已修):
...
|------|------|---------------|----------------------|
| `manager/deploy/env`(600) | manager token 单一事实源,生产 .env 与 dsh-runtime 凭它生成 | `pct exec <CTID> -- cat /root/os-deploy/deploy/openshell-manager/.env > manager/deploy/env && chmod 600 manager/deploy/env` | ✅ 在位 |
...
| 各仓 `.agent/` 过程文档(engine 账本/决策/roadmap/研究、伞仓 status.md;退订 git) | AI 会话工作流事实源(任务包/ADR/证据索引),退订≠删除——文件全在本机 | 无"重建"——本机即唯一副本,跨机迁移须手工拷贝(禁入 git);各仓 `*.sh` 门禁脚本仍随仓,`.agent/*` 已 ignore | ✅ 在位(本机) |
