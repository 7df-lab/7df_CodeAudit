...
| `env.template` | JWT 密钥、网关发布端口、manager token 的**模板**。现行 deploy.sh 不消费本地 `env`：JWT 首次随机生成后驻留远端 `$DEPLOY_DIR/.env`（重复 deploy 不轮换），manager token 单一事实源 = `manager/deploy/env`（2026-09-13 审计勘误；若手工重建本地 `env` 密钥文件，伞仓 .gitignore 已防御性忽略之） |
...
或伞仓统一分发器：`bash deploy/sandbox-deploy.sh codeaudit [deploy|status|…]`（伞仓根执行）。
首次 deploy 需拉中间件（postgres/redis/minio/kafka）与 golang 镜像并做 7 次 Go 构建（七服务），
耗时较长（后续增量构建快）；中间件数据在 named volume，deploy 不清数据。

## 宿主机残留进程退役（2026-09-05 收敛时的历史记录）
