// axios adapter 层伪造网关——核心（自 fakeGateway.ts 抽出 2026-09-13）：
// 测试台（vitest beforeEach/afterEach 挂卸）与 dev:mock 视觉走查入口（demoGateway）
// 共用同一段路由匹配/回放逻辑。纪律沿用 ADR-203 测试台：
...
export type RouteValue = RouteHandler | unknown;
...
// 构造 axios adapter：按路由表匹配请求并回放载荷/错误；requests 数组由调用方持有
// （测试台按用例清空做隔离；demoGateway 留全程供控制台观察）。
export function buildGatewayAdapter(routes: Record<string, RouteValue>, requests: GatewayRequestLog[]): AxiosAdapter {
  return async (config: InternalAxiosRequestConfig): Promise<AxiosResponse> => {
    ...
    if (!hit) {
      throw new Error(
        `mockGateway: 未建模路由 ${method} ${path} —— 补 handler，或确认该链路不应出现（ADR-203 响亮失败纪律）`,
      );
    }
