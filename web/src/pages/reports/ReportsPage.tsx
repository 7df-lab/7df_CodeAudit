...
import { api, errStatus, listAllProjects, openReportWindow, regenerateReport } from '../../api/client';
...
const fmtLabel = (f: string) => REPORT_FORMAT[f] ?? '—'; // UNSPECIFIED/历史未记录 → —（不显示"未知"误导）
...
export default function ReportsPage() {
  ...
  // （审计修复/D4 裁定接线）：重新生成此前 mutation 挂空无按钮——
  ...
  return (
    <div>
      <PageHeader
        title={<>报告中心 {taskFilter && (
          <Tag closable color="blue" onClose={() => setParams({})}>
            任务：{taskFilter}
          </Tag>
        )}</>}
      />
      {/*  列表容器统一为裸 Table（此前 Reports/Users 包 Card、Projects/Tasks 裸放） */}
      <Table
        rowKey="report_id"
        loading={isLoading}
        ...
        locale={{ emptyText: <EmptyState description="暂无报告（任务完成后由编排器生成）" /> }}
          columns={[
            {
              ...
              render: (v: string) => (
                <Tooltip title={v}>
                  <Typography.Text style={{ wordBreak: 'break-all', fontFamily: MONO_FONT, fontSize: 13 }}>
            ...
            { title: '格式', dataIndex: 'format', render: (f: string) => fmtLabel(f) },
            { title: '生成时间', dataIndex: 'generated_at', render: (v: string | null) => (v ? dayjs(v).format('YYYY-MM-DD HH:mm:ss') : '—') },
            {
              ...
              render: (_: unknown, rec: ReportRow) => (
                <Space>
                  ...
                  {/* 接线：重新生成=重发 GenerateReport（幂等）；仅本条转圈 */}
                  <Button
                    size="small"
