...
- **创建/编辑类操作统一 Modal 形态**（2026-09-14 二期）：列表页 `PageHeader` 右上 primary
  按钮直开弹窗（新建项目/项目编辑/用户/Provider/新建任务一致），footer 集中主按钮，关闭
  即回收表单状态；新建任务为四步向导弹窗（width 760，Steps+策略卡内嵌，`/tasks/new` 深
  链由薄壳宿主页承接自动开弹窗）。
